...
def set_command(adb: AdbClient, value: int, *, execute: bool, confirm: str | None, backup_dir: Path) -> int:
    with tempfile.TemporaryDirectory(prefix="phnix-stat-maint-") as temp_name:
        ...
        try:
            ...
            # Re-read only after the service is gone so a final static_write_data()
            # cannot race with the host copy used for patching and rollback.
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Read or experimentally change PHNIX persistent statistics via ADB"
    )
    ...
    set_p = sub.add_parser(
        "set-mainboard-ota-count", help="set uint32 counter at statistics offset 0x24"
    )
