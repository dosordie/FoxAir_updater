...
future Windows frontend can pass the path to a bundled ``adb.exe`` while the
current Linux/Raspberry-Pi launcher passes ``adb`` or the VM simulator shim.
"""
...
class AdbClient:
    def __init__(self, executable: str | Path, serial: str | None = None):
    ...
    def popen_shell(self, command: str) -> subprocess.Popen:
        return subprocess.Popen([*self.base, "shell", command])
