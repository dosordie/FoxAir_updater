#!/usr/bin/env python3
"""Cross-platform maintenance tool for the persistent PHNIX statistics block.

This tool is intentionally separate from the OTA controller.  It talks to the
modem only through ADB and can therefore be called identically from Linux or
from the bundled Windows Python runtime.  The Windows GUI is only a frontend
for this command.
...
``/data/phnixIot_device_statisic`` file.  The original OTA controller is not
...
def first_pid(adb: AdbClient) -> int | None:
    pids = numeric_pids(adb.shell("pidof phnixIot4G || true", check=False))
...
def remote_exists(adb: AdbClient, path: str) -> bool:
    return adb.shell(f"test -e '{path}'; echo $?", check=False).splitlines()[-1:] == ["0"]
...
def preflight(adb: AdbClient, scratch: Path) -> dict:
    ...
    service_path = adb.shell(
        "p=$(pidof phnixIot4G | awk '{print $1}'); test -n \"$p\" && readlink /proc/$p/exe || true",
        check=False,
    )
    service_sha = adb.shell(f"sha256sum {REMOTE_SERVICE} | awk '{{print $1}}'", check=False).upper()
    tracer = adb.shell(
        "p=$(pidof phnixIot4G | awk '{print $1}'); test -n \"$p\" && awk '/^TracerPid:/ {print $2}' /proc/$p/status || true",
        check=False,
    )
...
def wait_service_gone(adb: AdbClient, old_pid: int, timeout: float = 8.0) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.25)
    raise MaintenanceError("phnixIot4G did not terminate after SIGTERM; no file was modified")
...
def wait_service_restored(adb: AdbClient, old_pid: int, timeout: float = 25.0) -> int:
    ...
    while time.monotonic() < deadline:
        ...
        if pid is not None and pid != old_pid:
            ...
            tracer = adb.shell(f"awk '/^TracerPid:/ {{print $2}}' /proc/{pid}/status || true", check=False)
...
def install_patched_file(adb: AdbClient, patched_local: Path, expected_sha: str) -> None:
    # Preserve original metadata by cloning the existing file first.  The local
    # payload is copied into that staged inode, then atomically renamed.
    adb.shell(
        f"rm -f {REMOTE_BACKUP} {REMOTE_STAGE} {REMOTE_PAYLOAD}; "
        f"cp -p {REMOTE_STATISTICS} {REMOTE_BACKUP} && cp -p {REMOTE_STATISTICS} {REMOTE_STAGE} && sync"
    )
...
def restore_remote_backup(adb: AdbClient) -> None:
    if remote_exists(adb, REMOTE_BACKUP):
        adb.shell(
            f"cp -p {REMOTE_BACKUP} {REMOTE_STAGE} && mv {REMOTE_STAGE} {REMOTE_STATISTICS} && "
...
def set_command(adb: AdbClient, value: int, *, execute: bool, confirm: str | None, backup_dir: Path) -> int:
    with tempfile.TemporaryDirectory(prefix="phnix-stat-maint-") as temp_name:
        temp = Path(temp_name)
        before = preflight(adb, temp)
        emit("preflight", **before)
        if not before["ok"]:
            raise MaintenanceError("preflight failed: " + ", ".join(name for name, ok in before["checks"].items() if not ok))
        if not execute:
        ...
        try:
            ...
            final_raw = pull_exact(adb, REMOTE_STATISTICS, temp / "final-statistics.bin", STATISTICS_SIZE)
            current_after_stop = counter_from_bytes(final_raw)
            backup_dir.mkdir(parents=True, exist_ok=True)
            stamp = time.strftime("%Y%m%d-%H%M%S")
            backup_path = backup_dir / f"phnixIot_device_statisic.{stamp}.before-counter-{current_after_stop}.bin"
            shutil.copy2(temp / "final-statistics.bin", backup_path)
            backup_sha = sha256_bytes(final_raw)
            emit(
                "state-backed-up",
            ...
            # Assert that exactly four bytes are permitted to differ.
            if final_raw[:MAINBOARD_OTA_OFFSET] != patched[:MAINBOARD_OTA_OFFSET] or final_raw[MAINBOARD_OTA_OFFSET + 4:] != patched[MAINBOARD_OTA_OFFSET + 4:]:
            ...
            emit("complete", value=value, file_value=file_value, ram_value=ram_value, backup=str(backup_path))
            return 0
        except BaseException:
            # Roll back only while the original process is still stopped.  Once
            # a new process has loaded the new structure, blindly restoring the
            # file would recreate the RAM/file race this command exists to avoid.
