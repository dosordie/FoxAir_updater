...
RUN_DIR="$LAB_ROOT/logs/cpin-emulator-$STAMP"
...
with open(from_app_path, "ab", buffering=0) as from_app, \
    ...
    transcript.write("RULES=AT+CGMM and AT+CPIN? only; everything else logged but unanswered\n")
    ...
    while True:
        ...
        while True:
            ...
            if reply is not None:
                ...
                transcript.write(f"REPLY_COUNT[{key}]={counts[key]}\n")
            else:
                transcript.write("MODEM -> APP: <no reply; unknown command>\n")
PY
chmod 0755 "$RUN_DIR/at_emulator.py"

log "Test 4: AT+CGMM und AT+CPIN? werden beantwortet"
printf 'CGMM: %s\n' "$MODEL"
...
case "$RC" in
  ...
  136) echo "SIGFPE; fuer diesen Test ist entscheidend, welche AT-Kommandos vorher erschienen." ;;
...
if [[ -s "$RUN_DIR/smd8-from-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-from-app.bin" | head -n 200
else
  echo '(keine Bytes)'
fi
...
if [[ -s "$RUN_DIR/smd8-to-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-to-app.bin" | head -n 120
else
...
A/B-Aenderung gegenueber Test 3:
- AT+CGMM bleibt wie zuvor emuliert
- neu: AT+CPIN? -> +CPIN: READY + OK
