...
# Synthetic lab ICCID only. Do not put a real SIM ICCID in the public repo.
ICCID="89860000000000000001"
STAMP="$(date +%Y%m%d-%H%M%S)"
RUN_DIR="$LAB_ROOT/logs/ccid-emulator-$STAMP"
...
peer, from_app_path, to_app_path, transcript_path, model, iccid = sys.argv[1:]
...
with open(from_app_path, "ab", buffering=0) as from_app, \
    ...
    transcript.write("RULES=AT+CGMM, AT+CPIN?, AT+CCID only; everything else logged but unanswered\n")
...
log "Test 5: CGMM + CPIN + CCID werden beantwortet"
printf 'CGMM: %s\n' "$MODEL"
...
' bash "$ROOTFS" "$RUN_DIR" "$RUN_SECS" "$MODEL" "$ICCID"
RC=$?
set -e
...
if [[ -s "$RUN_DIR/smd8-from-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-from-app.bin" | head -n 240
else
  echo '(keine Bytes)'
fi
...
if [[ -s "$RUN_DIR/smd8-to-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-to-app.bin" | head -n 160
else
...
A/B-Aenderung gegenueber Test 4:
- CGMM und CPIN bleiben wie zuvor emuliert
- neu: AT+CCID -> synthetische ICCID + OK
