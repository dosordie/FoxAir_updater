...
RUN_DIR="$LAB_ROOT/logs/cgdcont6-emulator-$STAMP"
...
cgdcont1 = f'AT+CGDCONT=1,"IPV4V6","{apn}"'.encode('ascii')
cgdcont6 = f'AT+CGDCONT=6,"IPV4V6","{apn}"'.encode('ascii')
replies = {
    b"AT+CGMM": ("\r\n" + model + "\r\n\r\nOK\r\n").encode("ascii"),
    b"AT+CPIN?": b"\r\n+CPIN: READY\r\n\r\nOK\r\n",
    b"AT+CCID": ("\r\n+CCID: " + iccid + "\r\n\r\nOK\r\n").encode("ascii"),
...
with open(from_app_path, "ab", buffering=0) as from_app, \
    ...
    transcript.write("RULES=CGMM, CPIN, CCID, CGDCONT context 1 and context 6; everything else logged but unanswered\n")
...
log "Test 7: zusaetzlich CGDCONT Kontext 6 beantworten"
printf 'CGMM: %s\n' "$MODEL"
printf 'CPIN: +CPIN: READY\n'
printf 'CCID: +CCID: %s (synthetisch)\n' "$ICCID"
printf 'CGDCONT: Kontext 1 + 6 | PDP=IPV4V6 | APN=%s | Antwort=OK\n' "$APN"
...
if [[ -s "$RUN_DIR/smd8-from-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-from-app.bin" | head -n 360
else
  echo '(keine Bytes)'
fi
...
if [[ -s "$RUN_DIR/smd8-to-app.bin" ]]; then
  xxd -g1 "$RUN_DIR/smd8-to-app.bin" | head -n 240
else
...
A/B-Aenderung gegenueber Test 6:
- CGMM, CPIN, CCID und CGDCONT Kontext 1 bleiben wie zuvor emuliert
- neu: exaktes AT+CGDCONT=6,"IPV4V6","orange.m2m.spec" -> OK
