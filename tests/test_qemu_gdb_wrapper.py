...
"""
        ...
        self.assertNotIn("hbreak *0x9a98", patched)
        self.assertNotIn("FOXAIR_QEMU_SYSTEM_BYPASS", patched)
        self.assertNotIn("shell rm -f /cache/phnixIot_device_OTA", patched)
        self.assertNotIn("shell : > /data/phnixIot_device_OTA_INFO", patched)
        self.assertIn(
            'printf "PHNIX post-parser pc=0x%x\\n", $pc', patched
        )
